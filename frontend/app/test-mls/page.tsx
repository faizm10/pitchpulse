...
const MLS_GAMES = [
  { id: '401871128', slug: 'atl-orl', label: 'ATL vs ORL', venue: { name: 'Mercedes-Benz Stadium', city: 'Atlanta, GA', longitude: -84.4004, latitude: 33.7553 } },
  { id: '401871127', slug: 'hou-stl', label: 'HOU vs STL', venue: { name: 'Shell Energy Stadium', city: 'Houston, TX', longitude: -95.3510, latitude: 29.7522 } },
];
...
function MLSMarkers({
  ...
  games: typeof MLS_GAMES;
  matchDataMap: Record<string, MLSMatchData | null>;
  activeIdx: number;
  ...
  return (
    <>
      {games.map((game, idx) => {
        const match = matchDataMap[game.id];
            ...
            <MarkerPopup>
              <div style={{ minWidth: 220, borderRadius: 12, overflow: 'hidden', background: 'var(--paper)' }}>
                ...
                <div style={{ padding: 12, background: 'var(--paper-2)' }}>
                  ...
                  {match && (
                    <div style={{ borderTop: '1px solid var(--rule-soft)', paddingTop: 8, marginTop: 4 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                        <span style={{ fontSize: 10, fontFamily: 'var(--mono)', letterSpacing: '0.1em', color: isLive ? 'var(--live)' : 'var(--ink-3)', display: 'flex', alignItems: 'center', gap: 4 }}>
                          {isLive && <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--live)', display: 'inline-block' }} />}
                          {isLive ? `LIVE${match.liveClock ? ` · ${match.liveClock}` : ''}` : match.state === 'post' ? 'FT' : 'PRE'}
                        </span>
                      </div>
  ...
  return (
    <div style={{ position: 'relative', height: isMobile ? 260 : 380, borderRadius: 12, overflow: 'hidden', border: '1px solid var(--rule)' }}>
      ...
      <Map center={[-89, 32]} zoom={isMobile ? 4 : 4.8} minZoom={3} maxZoom={12} theme="light">
        ...
        <MLSMarkers games={MLS_GAMES} matchDataMap={matchDataMap} activeIdx={activeIdx} onSelect={onSelect} />
...
function ScoreHero({ match, liveClock, isMobile }: { match: MLSMatchData; liveClock: string; isMobile: boolean }) {
  ...
  const nameFontSize = isMobile ? 18 : 30;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 16, minWidth: 0 }}>
        <img src={homeTeam.logo} alt={homeTeam.name} width={logoSize} height={logoSize}
          style={{ width: logoSize, height: logoSize, objectFit: 'contain', flexShrink: 0 }}
          onError={(e) => { (e.target as HTMLImageElement).style.opacity = '0.3'; }} />
        <div style={{ minWidth: 0 }}>
          <div className="serif" style={{ fontSize: nameFontSize, lineHeight: 1.05, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {homeTeam.name}
          </div>
          {!isMobile && (
        ...
        {state !== 'pre' && (
          <div className="serif tnum" aria-hidden="true" style={{ fontSize: scoreFontSize, lineHeight: 1, marginLeft: isMobile ? 4 : 12, flexShrink: 0 }}>
            {homeTeam.score}
          </div>
        )}
      </div>
      ...
      {/* Centre */}
      <div style={{ textAlign: 'center', minWidth: isMobile ? 64 : 100, flexShrink: 0 }}>
        {state === 'in' && isHalftime && (
          <>
        ...
        {state === 'in' && !isHalftime && (
          <>
            <div className="mono" style={{ fontSize: 10, color: 'var(--live)', letterSpacing: '0.22em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
            ...
            <div className="mono" aria-live="polite" aria-atomic="true" aria-label={`Match clock: ${clockDisplay}`}
          ...
          <>
            ...
            {showCountdown ? (
              <>
                ...
                <div className="mono tnum" aria-live="polite" style={{ fontSize: isMobile ? 22 : 28, marginTop: 4, fontVariantNumeric: 'tabular-nums', color: 'var(--live)' }}>
                  {countdownStr}
                </div>
              </>
            ) : (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 16, flexDirection: 'row-reverse', minWidth: 0 }}>
        <img src={awayTeam.logo} alt={awayTeam.name} width={logoSize} height={logoSize}
          style={{ width: logoSize, height: logoSize, objectFit: 'contain', flexShrink: 0 }}
          onError={(e) => { (e.target as HTMLImageElement).style.opacity = '0.3'; }} />
        ...
        {state !== 'pre' && (
          <div className="serif tnum" aria-hidden="true" style={{ fontSize: scoreFontSize, lineHeight: 1, marginRight: isMobile ? 4 : 12, flexShrink: 0 }}>
            {awayTeam.score}
          </div>
        )}
      </div>
...
function EventsFeed({ events, homeTeam, awayTeam, isMobile }: { events: KeyEvent[]; homeTeam: MLSTeam; awayTeam: MLSTeam; isMobile: boolean }) {
    ...
    <section aria-label="Key match events" style={{ padding: isMobile ? '20px 16px' : '28px 40px', borderTop: '1px solid var(--rule)' }}>
      ...
      <ol style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
        {events.map((ev) => {
          ...
          return (
            <li key={ev.id} aria-label={`${ev.clock} — ${ev.typeText} — ${athlete} (${ev.teamName})`}
              style={{
                display: 'grid', gridTemplateColumns: isMobile ? '36px 1fr' : '44px 1fr',
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  {isGoal && ev.homeScore != null && ev.awayScore != null && (
                    <span className="mono tnum" style={{ fontSize: 11, padding: '1px 7px', borderRadius: 3, background: teamColor, color: '#fff', letterSpacing: '0.05em' }}>
                ...
                {ev.fullText && ev.fullText !== ev.text && (
                  <div className="mono" style={{ fontSize: 10, color: 'var(--ink-3)', marginTop: 3, lineHeight: 1.5, fontStyle: 'italic' }}>{ev.fullText}</div>
                )}
              </div>
            </li>
          );
...
function StatsPanel({ teamStats, homeTeam, awayTeam, isMobile }: { teamStats: TeamStat[]; homeTeam: MLSTeam; awayTeam: MLSTeam; isMobile: boolean }) {
...
function LeadersSection({ leaders, isMatchLeaders, isMobile }: { leaders: TeamLeader[]; isMatchLeaders: boolean; isMobile: boolean }) {
  const teams = leaders.filter(t => t.categories.length > 0);
  if (!teams.length) return null;

  return (
    <section aria-label={isMatchLeaders ? 'Match leaders' : 'Season leaders'} style={{ padding: isMobile ? '20px 16px' : '28px 40px', borderTop: '1px solid var(--rule)' }}>
      <h2 className="mono" style={{ fontSize: 10, letterSpacing: '0.2em', color: 'var(--ink-3)', margin: '0 0 16px', paddingBottom: 10, borderBottom: '1px solid var(--rule)' }}>
...
function ConferenceTable({
  ...
  const cols = showWDL ? '20px 16px 1fr 32px 32px 32px 32px 32px 44px' : '20px 16px 1fr 28px 32px 44px';
  const headers = showWDL ? ['', '', 'Club', 'P', 'W', 'D', 'L', 'GD', 'Pts'] : ['', '', 'Club', 'P', 'GD', 'Pts'];
...
function ConferenceStandings({
  ...
  return (
    <section aria-label="MLS standings" style={{ padding: isMobile ? '20px 16px' : '28px 40px', borderTop: '1px solid var(--rule)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--rule)', paddingBottom: 10, marginBottom: 20, flexWrap: 'wrap', gap: 6 }}>
...
function GamePanel({ gameId, isMobile }: { gameId: string; isMobile: boolean }) {
  ...
  const fetchMatch = useCallback(async () => {
    try {
      const res = await fetch(`/api/test-mls/${gameId}`);
  ...
  if (loading) {
    return (
      <div style={{ padding: isMobile ? '40px 16px' : '60px 40px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <p className="serif it" style={{ fontSize: 20, color: 'var(--ink-3)', margin: 0 }}>Loading match…</p>
      </div>
    );
  }
  ...
  if (error || !match) {
    return (
      <div style={{ padding: isMobile ? '40px 16px' : '60px 40px' }} role="alert">
        <p className="serif" style={{ fontSize: 20, margin: 0 }}>Could not load match</p>
        <p className="mono" style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 8 }}>{error}</p>
        <button className="btn" onClick={fetchMatch} style={{ marginTop: 16, minHeight: 44, padding: '0 20px' }}>Retry</button>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Source bar */}
      <div style={{ padding: isMobile ? '8px 16px' : '10px 40px', borderBottom: '1px solid var(--rule)', background: 'var(--paper-2)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div className="mono" style={{ fontSize: 9, padding: '2px 7px', borderRadius: 3, background: 'rgba(255,60,60,0.12)', color: '#e63c3c', letterSpacing: '0.1em' }}>ESPN</div>
          {isHybrid && <div className="mono" style={{ fontSize: 9, padding: '2px 7px', borderRadius: 3, background: 'rgba(0,120,255,0.12)', color: '#0078ff', letterSpacing: '0.1em' }}>+ FOTMOB</div>}
          {match.venue.name && (
          ...
          {match.broadcast && (
            <span className="mono" style={{ fontSize: 9, color: 'var(--ink-3)', letterSpacing: '0.1em' }}>· {match.broadcast}</span>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button className="btn" onClick={fetchMatch} aria-label="Refresh" style={{ fontSize: 10, padding: '0 12px', minHeight: 36 }}>
            Refresh
          </button>
        </div>
      </div>
      ...
      {match.keyEvents?.length > 0 && (
        <EventsFeed events={match.keyEvents} homeTeam={match.homeTeam} awayTeam={match.awayTeam} isMobile={isMobile} />
      )}

      {match.teamStats?.length > 0 && (
        <StatsPanel teamStats={match.teamStats} homeTeam={match.homeTeam} awayTeam={match.awayTeam} isMobile={isMobile} />
      )}

      {match.teamLeaders?.length > 0 && (
        <LeadersSection leaders={match.teamLeaders} isMatchLeaders={match.isMatchLeaders} isMobile={isMobile} />
      )}
...
export default function TestMLSPage() {
  ...
  // Lightweight match previews for the map (fetched separately)
  const [mapData, setMapData] = useState<Record<string, MLSMatchData | null>>({});

  useEffect(() => {
    const load = async () => {
      const results = await Promise.allSettled(
        MLS_GAMES.map(g => fetch(`/api/test-mls/${g.id}`).then(r => r.json()))
      );
      const next: Record<string, MLSMatchData | null> = {};
      results.forEach((r, i) => {
        next[MLS_GAMES[i].id] = r.status === 'fulfilled' ? (r.value?.match ?? null) : null;
      });
      setMapData(next);
    };
    load();
  ...
  const activeGame = MLS_GAMES[activeIdx];
    ...
    <div className="screen" style={{ minHeight: '100vh', maxWidth: '100vw', overflowX: 'hidden' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div className="mono" style={{ fontSize: 9, padding: '2px 7px', borderRadius: 3, background: 'rgba(255,60,60,0.12)', color: '#e63c3c', letterSpacing: '0.1em' }}>
            ESPN + FOTMOB
          </div>
        </div>
        <div className="mono" style={{ fontSize: 9, padding: '2px 8px', borderRadius: 3, background: 'rgba(255,200,0,0.15)', color: '#b8860b', letterSpacing: '0.12em', border: '1px solid rgba(200,160,0,0.3)' }}>
          DEV
        </div>
      </header>

      {/* USA map */}
      <div style={{ padding: isMobile ? '16px 16px' : '20px 40px', borderBottom: '1px solid var(--rule)' }}>
        <MLSMap matchDataMap={mapData} activeIdx={activeIdx} isMobile={isMobile} onSelect={setActiveIdx} />
      </div>
      ...
      <div style={{
        ...
        {MLS_GAMES.map((game, idx) => {
          const md = mapData[game.id];
          const isLive = md?.state === 'in';
          const isActive = idx === activeIdx;
          ...
          return (
            <button
              ...
              style={{
                ...
                border: 'none', borderBottom: isActive ? '2px solid var(--pulse)' : '2px solid transparent',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {isLive && (
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--live)', display: 'inline-block', flexShrink: 0 }} />
                )}
                <span className="mono" style={{ fontSize: 11, letterSpacing: '0.12em', color: isActive ? 'var(--ink)' : 'var(--ink-3)' }}>
                  {md ? `${md.homeTeam.abbreviation} vs ${md.awayTeam.abbreviation}` : game.label}
                </span>
              </div>
              <div className="mono" style={{ fontSize: 9, color: 'var(--ink-3)', marginTop: 3, letterSpacing: '0.1em' }}>
                {game.venue.city}
              </div>
              {md && md.state !== 'pre' && (
                <div className="mono tnum" style={{ fontSize: 13, marginTop: 4, fontVariantNumeric: 'tabular-nums', color: isActive ? 'var(--ink)' : 'var(--ink-3)' }}>
                  {md.homeTeam.score} – {md.awayTeam.score}
                </div>
              )}
            </button>
          );
      ...
      {/* Active game panel */}
      <GamePanel key={activeGame.id} gameId={activeGame.id} isMobile={isMobile} />
